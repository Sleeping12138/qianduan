        ...
        <div class="box_8 flex-col">
            <div class="group_12 flex-row justify-between">
                <div class="image-wrapper_1 flex-row">
                    ...
                    <div class="question-nav" @click="getPreQuestion">
                        <el-icon>
                            <span>上一题</span>
                            <ArrowLeft />
                        </el-icon>
                    </div>
                    <div class="question-nav" @click="getNextQuestion">
                        <el-icon>
                            <ArrowRight />
                            <span>下一题</span>
                        </el-icon>
...
import { getQuestionInfo, preQuestion, nextQuestion } from '@/api/question.js';
import { getQuestionOfFirst, examPreQuestion, examNextQuestion } from '@/api/exam.js';
import { submitRabbitMQ, getExeResult } from '@/api/user.js';
...
async function getQuestionDetail() {
    if (examId && (questionId == null || questionId == '')) {
        let ret = await getQuestionOfFirst({ examId })
        questionId = ret.data
    }
 ...
 * 获取上一题
 */
async function getPreQuestion() {
    if (examId) {
        let ret = await examPreQuestion(examId, questionId)
        questionId = ret.data;
    } else {
        const res = await preQuestion({ questionId })
        questionId = res.data
    }
    getQuestionDetail()
}
...
async function getNextQuestion() {
    if (examId) {
        let ret = await examNextQuestion(examId, questionId)
        questionId = ret.data;
    } else {
        const res = await nextQuestion({ questionId })
        questionId = res.data
    }
    getQuestionDetail()
}
...
async function submitQuestion() {
    ...
    await submitRabbitMQ(submitDTO)
...
async function getQuestionResult() {
    const res = await getExeResult(submitDTO.examId, submitDTO.questionId, currentTime)
