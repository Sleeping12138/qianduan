...
import { getExamListFromRedis, enter, getExamRankInfo } from '@/api/exam.js';
...
async function getExamRankList() {
    const result = await getExamRankInfo(rankParams)
    examRankList.value = result.data.dataList
    rankTotal.value = result.data.total
}
