...
<script setup>
import { getUserDetailInfo, editUserInfo, headImageUpdate, getLoginUserInfo } from '@/api/user.js'
...
async function editUser() {
    await editUserInfo(userDetailForm)
    getUserDetail()
}
...
/**
 * 由于upload组件不会使用axios进行交互，所以要定义请求头
 */
const headers = ref({
...
async function handleUploadSuccess(res) {
    if (res.code !== 200) {
        ...
        console.log("success");
        console.log(res);
        const HeadImageUpdateDTO = reactive({
            headImage: res.data.name
        })
        await headImageUpdate(HeadImageUpdateDTO)
        getUserDetail()
        getLoginUserInfo()
