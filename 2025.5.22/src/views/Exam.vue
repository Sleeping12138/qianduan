        ...
        <el-table-column label="操作" width="180px">
            <template #default="{ row }">
                ...
                <el-button v-if="row.status == 1 && isNotStartExam(row)" type="text"
                    @click="cancelPublishExamById(row.examId)">撤销发布</el-button>
                <el-button v-if="row.status == 0 && isNotStartExam(row)" type="text"
                    @click="publishExamById(row.examId)">发布</el-button>
...
import { getExamList, deleteExam, publishExam, cancelPublishExam } from '@/api/exam.js';
...
async function onEdit(examId) {
    router.push(`/oj/layout/updataExam?type=edit&examId=${examId}`)
}
 ...
 * @param examId 
 */
async function publishExamById(examId) {
 ...
 * @param examId 
 */
async function cancelPublishExamById(examId) {
    await cancelPublishExam(examId);
    getList();
